...
int bt_bap_scan_delegator_mod_src(const struct bt_bap_scan_delegator_mod_src_param *param)
{
	...
	if (internal_state == NULL) {
		LOG_DBG("Could not find receive state with src_id %u",
			param->src_id);
					 ...
					 void *user_data)
{
	for (size_t i = 0U; i < ARRAY_SIZE(scan_delegator.recv_states); i++) {
		if (scan_delegator.recv_states[i].active) {
			...
			stop = func(&scan_delegator.recv_states[i].state,
				    user_data);
			if (stop) {
				return;
			}
...
static bool scan_delegator_state_find_state_cb(
	const struct bt_bap_scan_delegator_recv_state *recv_state,
	void *user_data)
{
	...
	bt_bap_scan_delegator_state_func_t func, void *user_data)
{
	...
	bt_bap_scan_delegator_foreach_state(scan_delegator_state_find_state_cb,
					    &param);
